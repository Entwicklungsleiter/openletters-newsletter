...
class NewsletterEditor extends Parentclass {
    ...
    protected function edit() {
        ...
        if (array_key_exists("sendNewsletter", $_GET)) {
            if (array_key_exists("sendNewsletterConfirm", $_GET) 
                && $_GET['sendNewsletterConfirm'] == 1) {
        ...
        if (count($params) == 1) {
            ...
            if (!array_key_exists("useRawHtml", $_GET) || $_GET['useRawHtml'] != 'true') {
                if (isset($_GET['editEntry'])) {
                    // jQuery laden
                    $tag = "<script type=\"text/javascript\" src=\"" . ROOT_PATH . "vendor/jquery/jquery-1.3.2.min.js\"></script>";
    ...
    protected function init_tinymce_editor() {
        ...
        if (isset($_GET['useRawHtml']) 
            && $_GET['useRawHtml'] == 'true') {
            return( $rueckgabe);
        }

        return( $this->execute_view("edit_entry_init_tinymce.view.php"));
    }

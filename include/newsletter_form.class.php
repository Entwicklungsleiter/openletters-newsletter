...
class NewsletterForm extends Parentclass
{
    ...
    protected function execute_double_optin($double_optin_token)
    {
        ...
        if ($erg && is_array($erg) && sizeof($erg) > 0) {
            ...
            if (FALSE !== $this->db->prepare_and_execute($sql, array("email_id" => $erg[0]['email_id'], "updatedAt" => date('Y-m-d H:i:s')))
            ) {
     ...
     */
    protected function send_double_optin_mail($firstname, $name, $email, $double_optin_token)
    {
        $rueckgabe = false;
        ...
        try {
            ...
            $url = rtrim(ROOT_DOMAIN, "/") . "/index.php?doubleoptin=" . $double_optin_token;
            ...
            if (DEBUG_MODUS > 0) {
                $this->logmessage("Die Double-OptIn-Mail an " . $firstname . ' ' . $name . " (" . $email . ") wurde simuliert.");
                file_put_contents(ROOT_PATH . "/uploaded/" . time() . "_" . str_replace("@", "_at_", $email) . ".html", $text);
                $rueckgabe = true;
            } elseif ($mail->send()) {
                $rueckgabe = true;
            }
    ...
    public function subscribe($params = null)
    {
        ...
        if ($this->user_exists($params['newsemail'])) {
            $rueckgabe = "<p>" . $this->text->get_text("newsletter_subscription_not_needed") . "</p>\n";
        } else {

            try {
                $this->insert_user($params['firstname'], $params['name'], $params['title'], $params['newsemail'], true);
                $rueckgabe = $this->execute_view(ROOT_PATH . "include/newsletter_registerform_success.view.php");
            } catch (Exception $e) {
                $this->logmessage('Exception: ' . $e->getMessage() . ' in ' . $e->getFile() . $e->getLine() . ' Trace:' . $e->getTraceAsString());
    ...
    public function insert_user($firstname, $name, $title, $email, $send_double_optin)
    {

        // Mailadresse existiert noch nicht: Werte vorbereiten
        $db_parameters = array(
            'name' => $firstname ? $firstname . " " . $name : $name,
            'title' => $title,
        ...
        if ($send_double_optin) {
            $double_optin_token = sha1(microtime(true) . $email);
            $db_parameters['double_optin_token'] = $double_optin_token;
            $db_parameters['aktiv'] = 0;
        }

        if (!$send_double_optin || $this->send_double_optin_mail($firstname, $name, $email, $double_optin_token)) {
            ...
            $message = sprintf("Double-OptIn mail to User %s %s with email %s could not be send.", $firstname, $name, $email);
    ...
    public function user_exists($email_adress)
    {

        // Emailadresse suchen
        $email_adress = $this->db->prepare_for_db($email_adress);
        $sql = "SELECT name FROM " . PREFIX . "newsletter WHERE email=:newsemail";
        $res = $this->db->prepare_and_execute($sql, array("newsemail" => $email_adress));

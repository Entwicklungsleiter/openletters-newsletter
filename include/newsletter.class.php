...
require_once( ROOT_PATH . "include/newsletter_imap.class.php");
...
//			$this->template = new NewsletterTemplate( "template_01", $erg[0]['date']);
    ...
    public function add_entry()
    {
        ...
        $this->db->prepare_and_execute("INSERT INTO " . PREFIX . "newsletterEntries "
            . "SET newsletterContId=:newsletterContId, headline='Neuer Eintrag', content='<p></p>', ordering=:ordering", array("newsletterContId" => $this->newsletter_id, "ordering" => count($size)));
    ...
    {
        $this->db->prepare_and_execute("DELETE FROM " . PREFIX . "newsletterCont WHERE id=:newsletter_id;", array("newsletter_id" => $this->newsletter_id));
        $this->db->prepare_and_execute("DELETE FROM " . PREFIX . "newsletterEntries WHERE newsletterContId=:newsletterContId", array("newsletterContId" => $this->newsletter_id));
    ...
    public function delete_entry($entry_id)
    {
        ...
        $this->db->prepare_and_execute("DELETE FROM " . PREFIX . "newsletterEntries "
            . "WHERE id=:entry_id AND newsletterContId=:newsletter_id;", array("entry_id" => $entry_id, "newsletter_id" => $this->newsletter_id));
    ...
    protected function execute_sending($receivers)
    {
        ...
        for ($i = 0; $i < $anzahl; $i++) {
            ...
            try {
                ...
                $html_text = $this->getHtml($receivers[$i]);
                $plain_text = $this->getTxt($receivers[$i]);
                ...
                if (DEBUG_MODUS > 0) {
                    ...
                    file_put_contents(ROOT_PATH . "/uploaded/" . time() . "_" . str_replace("@", "_at_", $receivers[$i]['email']) . ".txt", $plain_text);
                    file_put_contents(ROOT_PATH . "/uploaded/" . time() . "_" . str_replace("@", "_at_", $receivers[$i]['email']) . ".html", $html_text);
                } else {
                    ...
                    if (!$mail->send()) {
                        ...
                        if ($imap instanceof NewsletterImap && $imap->folderExists(IMAP_DRAFT)) {
                            $imap->saveMailToFolder($mail, IMAP_DRAFT);
                        }
                    } elseif ($imap instanceof NewsletterImap && $imap->folderExists(IMAP_SENT)) {
     ...
     * @param array $receiver die DB-Daten des Empfaengers dieses Newsletters
     * @return string der Newsletter als HTML
     */
    public function getHtml($receiver = null)
    {
        ...
        $template = $this->replacePlaceholdersInEmail($template, $receiver);
        return( $template);
    }
     ...
     * fertig, lediglich der/die Platzhalter fuer die E-Mail-Adresse des Empfaengers
     * ist noch nicht ersetzt oder entfernt.
     * @param string $receiver die DB-Daten des Empfaengers dieses Newsletters
     * @return string der Newsletter als HTML
     */
    public function getTxt($receiver = null)
    {
        // fast fertiges Template wieder aufnehmen oder Template erstmal aus der TemplateKlasse holen
    ...
    protected function replacePlaceholdersInEmail($email, $receiver)
    {
    ...
    public function send($return_ajax = 0)
    {
        ...
        if ($return_ajax != 1) {
            // jQuery laden
            $tags = array();
            $tags[] = "<script type=\"text/javascript\" src=\"" . ROOT_PATH . "vendor/jquery/jquery-1.3.2.min.js\"></script>";
            $tags[] = "<script type=\"text/javascript\" src=\"" . ROOT_PATH . "vendor/jqueryui/js/jquery-ui-1.7.2.custom.min.js\"></script>";
            $tags[] = "<link rel=\"stylesheet\" type=\"text/css\" href=\"" . ROOT_PATH . "vendor/jqueryui/css/ui-lightness/jquery-ui-1.7.2.custom.css\" />";
            $tags[] = "<script type=\"text/javascript\" src=\"" . ROOT_PATH . "vendor/jquery.timeout_interval_idle-0.5.js\"></script>";
    ...
    public static function initMailer($port = null, $security = null)
    {
        ...
        /** PhpMailer einbinden */
        require_once( realpath(ROOT_PATH) . '/vendor/phpmailer/class.phpmailer.php');
        require_once( realpath(ROOT_PATH) . "/vendor/phpmailer/class.smtp.php");
